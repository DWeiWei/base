...
//页面列表
    ...
    $('.btn-add').off('click').on('click', function () {
        ...
        $("#fruitTrees-modal").modal({backdrop: 'static', keyboard: false}, 'show');
    });
    //编辑
    $("#page_list").off('click', '.btn-edit').on('click', '.btn-edit', function () {
        ...
        Request.get('fruitTrees/' + id, {}, function (e) {
            if (e.success) {
    ...
    $('#page_list').off('click', '.btn-del').on('click', '.btn-del', function () {
        ...
        dialog.delete('警告', '真的要删除吗？', function () {
            Request.delete('fruitTrees/' + id, {}, function (e) {
                if (e.success) {
        ...
        if (type === 0) {
            ...
            initFileInput(fileImg, fileInputOpts,function(event, data, previewId, index) {
                ...
                if (response.success) {
                    ...
                    $('#fruitTrees-modal #uploaded_image').val( data.md5 );
                }
            }, function (event, id, index) {
                $('#fruitTrees-modal #uploaded_image').val('');
            });
            ...
            if (obj != null && obj.imageUrl != null) {
                ...
                $('#fruitTrees-modal #uploaded_image').val(obj.imageUrl);
            }
    ...
    //新增或者修改校验
    $("form#fruitTrees-form").validate({
        rules:{
            name:{required: true},
        },
        messages: {
            name: {required: "请输入果树名称"},
        },
        errorElement:'em',
        submitHandler: function(form){
            ...
            var requestType = $('#fruitTrees-modal').data('type') == '0';
            var uid = $('#fruitTrees-modal').data('id');
            var req = requestType ? Request.post : Request.put;

            req('fruitTrees/' + (requestType ? '' : uid), params, function (e) {
                if (e.success) {
                    toastr.info('保存成功', opts);
                    $('#fruitTrees-modal').modal('hide');
                    pageList.bootstrapTable('refresh');
                } else {
