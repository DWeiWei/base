...
//页面列表
    var pageList = $('#refund_list').bootstrapTable({
        url: "livestockOrder", //BASE_PATH + "farm/",
        ...
        ajax: function (params) {
            ...
            if (params.data.filter !== undefined && params.data.filter != "") {
                var type = JSON.parse(params.data.filter).type;
                str += '&terms[1].column=type&terms[1].value=' + type;
                //str += '&terms[1].column=remark&terms[1].value=' + aa + params.data.search + aa + '&terms[1].termType=like';
            }
            if (params.data.sort !== undefined && params.data.sort != '' && params.data.type !== undefined && params.data.type != '') {
                str += '&sorts[0].name=' + params.data.sort + '&sorts[0].type=' + params.data.type;
            }
