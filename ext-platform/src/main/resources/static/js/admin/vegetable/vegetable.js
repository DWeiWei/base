...
//页面列表
    ...
    $('.btn-add').off('click').on('click', function () {
        ...
        $("#vegetable-modal").modal({backdrop: 'static', keyboard: false}, 'show');
    });
    //编辑
    $("#page_list").off('click', '.btn-edit').on('click', '.btn-edit', function () {
        ...
        Request.get('vegetable/' + id, {}, function (e) {
            if (e.success) {
    ...
    $('#page_list').off('click', '.btn-del').on('click', '.btn-del', function () {
        ...
        dialog.delete('警告', '真的要删除吗？', function () {
            Request.delete('vegetable/' + id, {}, function (e) {
                if (e.success) {
        ...
        if (type === 0) {
            ...
            initFileInput(fileImg, fileInputOpts,function(event, data, previewId, index) {
                ...
                if (response.success) {
                    ...
                    $('#vegetable-modal #uploaded_image').val( data.md5 );
                }
            }, function (event, id, index) {
                $('#vegetable-modal #uploaded_image').val('');
            });
            ...
            if (obj != null && obj.imageUrl != null) {
                ...
                $('#vegetable-modal #uploaded_image').val(obj.imageUrl);
            }
    ...
    //新增或者修改校验
    $("form#vegetable-form").validate({
        rules:{
            name:{required: true},
        },
        messages: {
            name: {required: "请输入蔬菜名称"},
        },
        errorElement:'em',
        submitHandler: function(form){
            ...
            if(image==null || image==''){
                toastr.warning('提示', '还没上传图片呢!', function(){});
            ...
            var requestType = $('#vegetable-modal').data('type') == '0';
            var uid = $('#vegetable-modal').data('id');
            var req = requestType ? Request.post : Request.put;

            req('vegetable/' + (requestType ? '' : uid), params, function (e) {
                if (e.success) {
                    toastr.info('保存成功', opts);
                    $('#vegetable-modal').modal('hide');
