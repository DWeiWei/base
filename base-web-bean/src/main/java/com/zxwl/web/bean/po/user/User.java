...
public class User extends GenericPo<String> {
    ...
    public boolean isAdmin() {
        if (this.userRoles != null) {
            List<UserRole> roles = this.userRoles.stream()
                    .filter(role -> "admin".equals(role.getRole().getType())||"super-admin".equals(role.getRole().getType()))
     ...
     */
    @JSONField(serialize=false)
    public boolean isFarmAdmin() {
        if (this.userRoles != null) {
            List<UserRole> roles = this.userRoles.stream()
                    //.filter(role -> role.getRoleId().contains("admin"))
                    .filter(role -> "farm-admin".equals(role.getRoleId()))
                    .collect(Collectors.toList());
            if (roles != null && roles.size() > 0)
                return true;
        }
    ...
    public UserDefInfo getUserInfo() {
        return userInfo;
    }

    public void setUserInfo(UserDefInfo userInfo) {
