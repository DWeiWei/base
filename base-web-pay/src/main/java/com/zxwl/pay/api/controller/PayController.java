...
public class PayController {
    ...
    public Map toPay(Integer payId, String openid, BigDecimal price,String title,String outTradeNo) {
        //获取对应的支付账户操作工具（可根据账户id）
        PayResponse payResponse = service.getPayResponse(payId);

        PayOrder order = new PayOrder(title, "摘要", null == price ? new BigDecimal(0.01) : price, outTradeNo, PayType.valueOf(payResponse.getStorage().getPayType()).getTransactionType("JSAPI"));
